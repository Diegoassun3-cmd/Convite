...
import { CATEGORIAS, salvarUpload } from './upload.js';
...
app.post('/api/rsvp', async (c) => {
  const ok = await permitir(c.env.RATE_LIMIT, `rsvp:${ipDoVisitante(c)}`, 30, 600);
...
app.post('/api/admin/login', async (c) => {
  const ok = await permitir(c.env.RATE_LIMIT, `login:${ipDoVisitante(c)}`, 10, 900);
...
app.post('/api/admin/upload/:categoria', exigirLogin, async (c) => {
  ...
  try {
    const url = await salvarUpload(c.env.UPLOADS, categoria, arquivo);
...
// ---------------------------------------------------------------------
// Arquivos enviados (servidos a partir do R2)
// ---------------------------------------------------------------------
app.get('/uploads/:pasta/:nome', async (c) => {
  const chave = `${c.req.param('pasta')}/${c.req.param('nome')}`;
  const objeto = await c.env.UPLOADS.get(chave);
  if (!objeto) return c.notFound();
  c.header('Content-Type', objeto.httpMetadata?.contentType || 'application/octet-stream');
